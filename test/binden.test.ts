/* eslint-disable init-declarations, class-methods-use-this, @typescript-eslint/no-throw-literal, @typescript-eslint/no-floating-promises */
...
describe("Binden", () => {
  ...
  beforeEach(async () => {
    ...
    setGlobalDispatcher(
      new Agent({ keepAliveTimeout: 10, keepAliveMaxTimeout: 10 }),
    );
    app = new Binden();
  ...
  it(".createSecureServer()", async () => {
    ...
    newApp.use(m);
    const response = await fetch(path.toString(), { dispatcher });
  ...
  it("Middleware `ignore_errors === true`", async () => {
  ...
  it("404 error (context.done === false)", async () => {
    app.use(new CustomMiddleware());
    const response = await fetch(url);
    deepEqual(response.status, 404);
  });

  it("404 error (Empty `guarded` router)", async () => {
  ...
  it("405 error (`guarded` router)", async () => {
  ...
  it("405 error (`guarded` router `auto_head === false`)", async () => {
  ...
  it("custom `error_handler`", async () => {
    function error_handler(context: Context): void {
      context.response.status(401).setHeader("X-HEADER", "Value").end("Secret");
    }
  ...
  it("errorHadler (default error code when no `status`)", async () => {
    const status = 405;
    const message = "Hello World!";
    const expose = true;
  ...
  it("errorHadler (`message` is not a string)", async () => {
    const status = 405;
    const message = 1;
    const expose = true;
  ...
  it("errorHadler (`expose === true`)", async () => {
    const status = 405;
    const message = "Hello World!";
    const expose = true;
  ...
  it("errorHadler (`expose === false`)", async () => {
    const status = 405;
    const message = "Hello World!";
    const expose = false;
    app.use(new ErrorMiddleware({}, status, { message, expose }));
  ...
  it("errorHadler (custom `Content-Type`)", async () => {
    const status = 405;
  ...
  it("errorHadler (invalid `json`)", async () => {
    const status = 405;
    const message = "Something happend";
  ...
  it(".errorHandler() (`headersSent === true`)", async () => {
    const message = "<html></html>";
  ...
  it(".errorHandler() (`writableEnded === true`)", async () => {
    const expose = true;
    const status = 200;
    const message = "<html></html>";
    const html = "text/html";
